...
public class Utils {
    ...
    private static String getProxyViewActionByActivityType(int activityType) {
        ...
        switch (activityType) {
            case Constants.ACTIVITY_TYPE_NORMAL: {
                proxyViewAction = Constants.PROXY_ACTIVITY_VIEW_ACTION;
                break;
            }
            case Constants.ACTIVITY_TYPE_FRAGMENT: {
                proxyViewAction = Constants.PROXY_FRAGMENT_ACTIVITY_VIEW_ACTION;
                break;
            }
            case Constants.ACTIVITY_TYPE_PREFERENCE_ACTIVITY: {
                proxyViewAction = Constants.PROXY_PREFERENCE_ACTIVITY_VIEW_ACTION;
                break;
            }
            case Constants.ACTIVITY_TYPE_ACTIONBAR:
    ...
    private static int getActivityType(String className, ClassLoader classLoader) {
        int activityType = Constants.ACTIVITY_TYPE_UNKNOWN;
    ...
    private static int getActivityType(Class<?> cls) {
        ...
        try {
            if (cls.asSubclass(BasePluginActivity.class) != null) {
                activityType = Constants.ACTIVITY_TYPE_NORMAL;
                return activityType;
            }
        ...
        try {
            if (cls.asSubclass(BasePluginFragmentActivity.class) != null) {
                activityType = Constants.ACTIVITY_TYPE_FRAGMENT;
                return activityType;
            }
        } catch (ClassCastException e) {
            // ignored
