...
/**
 * Modify for PluginManager using the Application context replace the FragmentActivity context.
 *
 * @author singwhatiwanna, modify Nemo.
 * note: can use that like this.
 * All the time use 'that' represents the Proxy FragmentActivity Context.
 * Cause this method is final, so use that.requestWindowFeature(featureId).
 */

public class BasePluginFragmentActivity extends FragmentActivity implements IPluginActivity {

    private static final String TAG = BasePluginFragmentActivity.class.getSimpleName();
    ...
    public void attach(Activity proxyActivity, PluginPackage pluginPackage) {
        LOG.d(TAG, "attach: proxyActivity(FragmentActivity) = " + proxyActivity);
